export interface CategoryMeta {
  label: string;
  emoji: string;
  image: string;
  gradient: string;
  badge: string;
}

const u = (id: string, w = 1000) =>
  `https://images.unsplash.com/photo-${id}?auto=format&fit=crop&w=${w}&q=80`;

export const CATEGORY_META: Record<string, CategoryMeta> = {
  gastronomia: {
    label: "Gastronomía",
    emoji: "🥧",
    image: u("1569718212165-3a8278d5f624"),
    gradient: "from-[#7c3a1d] via-[#b85c3c] to-[#2a1207]",
    badge: "bg-[#b85c3c]/15 text-[#d47554]",
  },
  historia: {
    label: "Historia",
    emoji: "⛏️",
    image: u("1518199266791-5375a83190b7"),
    gradient: "from-[#5a4214] via-[#c8a356] to-[#1a1308]",
    badge: "bg-[#c8a356]/15 text-[#d4b26a]",
  },
  historico: {
    label: "Histórico",
    emoji: "🏛️",
    image: u("1518717758536-85ae29035b6d"),
    gradient: "from-[#4a3728] via-[#a9804f] to-[#1a1209]",
    badge: "bg-[#c8a356]/15 text-[#d4b26a]",
  },
  mineria: {
    label: "Minería",
    emoji: "⚒️",
    image: u("1506905925346-21bda4d32df4"),
    gradient: "from-[#3a2a1a] via-[#8a6a3a] to-[#120d06]",
    badge: "bg-[#c8a356]/15 text-[#d4b26a]",
  },
  museo: {
    label: "Museo",
    emoji: "🏺",
    image: u("1579783902614-a3fb3927b6a5"),
    gradient: "from-[#16243a] via-[#1e3a5f] to-[#0a0f18]",
    badge: "bg-[#1e3a5f]/40 text-[#7aa2d4]",
  },
  cultura: {
    label: "Cultura",
    emoji: "🎭",
    image: u("1579783902614-a3fb3927b6a5"),
    gradient: "from-[#1e2440] via-[#4a3a6a] to-[#0e0d18]",
    badge: "bg-[#6a5aa0]/25 text-[#b3a6e0]",
  },
  arquitectura: {
    label: "Arquitectura",
    emoji: "⛲",
    image: u("1558636508-e0db3814bd1d"),
    gradient: "from-[#3a2c18] via-[#b08a4a] to-[#150f07]",
    badge: "bg-[#c8a356]/15 text-[#d4b26a]",
  },
  plaza: {
    label: "Plaza",
    emoji: "🏞️",
    image: u("1516483638261-f4dbaf036963"),
    gradient: "from-[#2c3a24] via-[#6a8a5a] to-[#0f140d]",
    badge: "bg-emerald-900/40 text-emerald-300",
  },
  naturaleza: {
    label: "Naturaleza",
    emoji: "🌲",
    image: u("1501785888041-af3ef285b470"),
    gradient: "from-[#1e3a2a] via-[#3f7a52] to-[#0c140e]",
    badge: "bg-emerald-900/40 text-emerald-300",
  },
  turismo: {
    label: "Turismo",
    emoji: "🧭",
    image: u("1469854523086-cc02fe5d8800"),
    gradient: "from-[#1c3a4a] via-[#2f6a8a] to-[#0a1218]",
    badge: "bg-sky-900/40 text-sky-300",
  },
  tradicion: {
    label: "Tradición",
    emoji: "🕯️",
    image: u("1608198093002-ad4e005484ec"),
    gradient: "from-[#3a2a3a] via-[#8a5a8a] to-[#150e15]",
    badge: "bg-purple-900/40 text-purple-300",
  },
  musica: {
    label: "Música",
    emoji: "🎻",
    image: u("1507838153414-b4b953384aa6"),
    gradient: "from-[#3a1c2a] via-[#8a3a5a] to-[#14080c]",
    badge: "bg-rose-900/40 text-rose-300",
  },
  hospedaje: {
    label: "Hospedaje",
    emoji: "🛏️",
    image: u("1566073771259-6a8506099945"),
    gradient: "from-[#1c2a3a] via-[#3a5a8a] to-[#0a1018]",
    badge: "bg-indigo-900/40 text-indigo-300",
  },
  artesanias: {
    label: "Artesanías",
    emoji: "🧶",
    image: u("1563013544-824ae1b704d3"),
    gradient: "from-[#3a2414] via-[#a05a2a] to-[#160c04]",
    badge: "bg-orange-900/40 text-orange-300",
  },
  default: {
    label: "Real del Monte",
    emoji: "🌄",
    image: u("1519681393784-d120267933ba"),
    gradient: "from-[#3a3214] via-[#c8a356] to-[#120e05]",
    badge: "bg-[#c8a356]/15 text-[#d4b26a]",
  },
};

export const LANDSCAPE_HERO = u("1519681393784-d120267933ba", 1920);
export const LANDSCAPE_ALT = u("1501785888041-af3ef285b470", 1920);
export const STREET_HERO = u("1558636508-e0db3814bd1d", 1920);
export const MINE_HERO = u("1518199266791-5375a83190b7", 1920);
export const FOOD_HERO = u("1504674900247-0877df9cc836", 1920);
export const CULTURE_HERO = u("1513364776144-60967b0f800f", 1920);
export const ECONOMY_HERO = u("1462826303106-4d37545f2e9d", 1920);

export function categoryMeta(cat?: string | null): CategoryMeta {
  if (!cat) return CATEGORY_META.default;
  return CATEGORY_META[cat] ?? CATEGORY_META.default;
}

export function itemImage(cat: string | undefined, customUrl?: string | null): string | null {
  if (customUrl) return customUrl;
  return categoryMeta(cat).image;
}
