...
import { Check, Crown, Store, Pickaxe, Sparkles, ShieldCheck, Gift, Medal, ArrowRight } from "lucide-react";
...
export default function PremiumPlans() {
  ...
  const handleActivate = async (planId: string, isCommerce: boolean) => {
    ...
    try {
      if (isCommerce) {
        // For commerce plans, user needs to have or create a business first
        navigate("/registro-comercio?plan=" + planId);
        setBusy(null);
        return;
      }
      const fn = planId === "99" ? "create-premium-checkout" : "create-premium-checkout";
  ...
  const plans = tab === "usuarios" ? USER_PLANS : COMMERCE_PLANS;

  return (
    <MainLayout>
      <SEOMeta title="Planes Premium · RDM Digital" description="Planes premium para usuarios y comercios en Real del Monte. Desde $99 MXN/mes." />
      <section className="pb-24 pt-8">
        <div className="container mx-auto max-w-6xl px-6">
          {/* Header */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 text-center">
            <span className="mb-3 block font-mono text-xs uppercase tracking-widest text-primary">DOCUMENTO MAESTRO · Capítulo VII</span>
            ...
            <p className="mx-auto max-w-2xl text-muted-foreground text-sm">
              Elige el plan que mejor se adapte a ti. Los puntos de juegos se convierten en premios reales
              en comercios federados de Real del Monte. Los comercios acceden a la red de jugadores y
          ...
          <div className="mx-auto mb-10 flex w-fit rounded-full border border-border/40 bg-card/60 p-1">
            <button
              ...
              className={cn(
                ...
                tab === "usuarios" ? "bg-gold/20 text-gold shadow-sm" : "text-muted-foreground hover:text-foreground",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 rounded-full px-5 py-2 text-xs font-mono uppercase tracking-widest transition-all",
                tab === "comercios" ? "bg-gold/20 text-gold shadow-sm" : "text-muted-foreground hover:text-foreground",
              )}
            >
          ...
          <div className="grid gap-6 md:grid-cols-2 mx-auto max-w-4xl">
            {plans.map((plan, i) => {
              ...
              return (
                ...
                >
                  {/* Background gradient */}
                  <div className={cn("pointer-events-none absolute inset-0 bg-gradient-to-br", plan.accent)} />
                  ...
                  <div className="relative">
                    ...
                    <div className="mb-1 flex items-center gap-2">
                      <Icon className={cn("h-6 w-6", isHighlight ? "text-gold" : "text-muted-foreground")} />
                    ...
                    <div className="mb-6 flex items-baseline gap-1">
                      <span className={cn("text-5xl font-display font-bold", isHighlight ? "text-gradient-gold" : "text-foreground")}>
                        {plan.price}
                      </span>
                    ...
                    <ul className="mb-8 space-y-3">
                      {plan.features.map((f) => (
                        <li key={f} className="flex items-start gap-2 text-sm text-foreground/80">
                          <Check className={cn("mt-0.5 h-4 w-4 shrink-0", isHighlight ? "text-gold" : "text-teal")} />
                          <span>{f}</span>
                        </li>
                      ))}
                    </ul>
          ...
          >
            ...
            <h3 className="font-display text-lg font-bold text-foreground mb-2">¿Cómo funcionan los puntos?</h3>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Juega Memoria Minera y Trivia Territorial para ganar puntos canjeables.
              Los puntos se acreditan automáticamente al completar partidas.
              Canjea tus puntos por pastes, micheladas, joyería de plata, cenas románticas,
              noches de hospedaje y recorridos guiados en comercios federados de Real del Monte.
              Los premios son aportados por los propios comercios — la plataforma no retiene comisión.
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3 text-teal" /> Stock limitado</span>
              <span className="flex items-center gap-1"><Gift className="h-3 w-3 text-gold" /> Canje inmediato</span>
              <span className="flex items-center gap-1"><Medal className="h-3 w-3 text-electric" /> Puntos seguros</span>
            </div>
          </motion.div>

          <p className="mt-8 text-center text-xs text-muted-foreground">
            ¿Dudas? Visita las{" "}
            <button onClick={() => navigate("/faq")} className="text-gold underline-offset-2 hover:underline">
